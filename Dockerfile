...
RUN mkdir -p /tmp/.sdl && \
    ...
    mkdir build && \
    cd build && \
    cmake .. -DCMAKE_INSTALL_PREFIX=/tmp/.sdl/SDL_mixer-${SDL_MIXER_VERSION} -DCMAKE_BUILD_TYPE=Release -DSDL_SHARED=ON -DSDL_STATIC=ON && \
    make && \
    make install
